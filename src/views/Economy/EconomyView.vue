    ...
    <div class="container">
        ...
        <div :style="{ width: collapsed ? '64px' : '230px' }">
            <t-menu theme="light" default-value="/" :collapsed="collapsed" expandMutex>
                <!-- Logo -->
                <template #logo>
                    <t-icon name="money" :size="collapsed ? '30px' : '25px'" v-if="!collapsed" />

                    <h3 v-if="!collapsed">经济政治</h3>

                    <t-icon :name="iconName" :size="collapsed ? '30px' : '25px'" @click="changeCollapsed" />
                </template>

                <!-- 菜单组遍历 -->
                <t-menu-group v-for="(group, gIndex) in LeftEconomyMenuList" :key="gIndex" :title="group.title">
                    <!-- 一级菜单项遍历 -->
...
import { LeftEconomyMenuList } from '@/api/menu'
